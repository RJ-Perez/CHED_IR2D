...
def ranking_framework_option(
    ...
    return rx.el.div(
        ...
        class_name=rx.cond(
            is_selected,
            "relative p-6 rounded-3xl border-2 border-blue-600 bg-blue-50/30 shadow-lg shadow-blue-100/50 cursor-pointer transition-all transform scale-[1.02] w-full",
            "relative p-6 rounded-3xl border border-gray-200 bg-white hover:border-blue-300 hover:bg-gray-50/50 cursor-pointer transition-all hover:shadow-md w-full",
        ),
    )
...
def registration_form() -> rx.Component:
    return rx.el.div(
        ...
        class_name="bg-white p-6 md:p-8 rounded-3xl border border-gray-100 shadow-xl shadow-slate-200/50 animate-in fade-in slide-in-from-bottom-6 duration-500",
    )
    ...
    return rx.el.div(
        rx.el.div(
            rx.el.div(
                rx.el.div(
                    rx.icon(
                        "building-2", class_name="h-8 md:h-10 w-8 md:w-10 text-white"
                    ),
                    class_name="p-4 md:p-5 bg-blue-600 rounded-2xl md:rounded-3xl mr-0 md:mr-8 mb-6 md:mb-0 shadow-lg shadow-blue-200",
                ),
                rx.el.div(
                    rx.el.div(
                        rx.el.div(
                            rx.el.h3(
                                ...
                                class_name="text-xl md:text-2xl font-black text-gray-900 tracking-tight leading-snug",
                            ),
                            rx.el.div(
                                rx.el.div(
                            ...
                            class_name="flex flex-col mb-4 md:mb-0",
                        ),
                        rx.el.button(
                            rx.icon("refresh-cw", class_name="h-4 w-4 md:mr-2"),
                            rx.el.span("Change", class_name="hidden md:inline"),
                            on_click=HEIState.deselect_hei,
                            class_name="flex items-center px-3 md:px-4 py-2 text-xs font-bold text-slate-500 hover:text-blue-600 hover:bg-blue-50 rounded-xl border border-slate-200 transition-all",
                        ),
                        class_name="flex flex-col md:flex-row items-start md:items-center justify-between w-full mb-6 md:mb-8",
                    ),
                    rx.el.div(
                        ...
                        class_name="grid grid-cols-1 sm:grid-cols-2 gap-y-4 md:gap-y-6 md:gap-x-12 pt-6 md:pt-8 border-t border-slate-100",
                    ),
                    class_name="flex-1",
                ),
                class_name="flex flex-col md:flex-row items-start",
            ),
            class_name="p-6 md:p-10",
        ),
        class_name="bg-white rounded-2xl md:rounded-3xl border border-slate-100 md:border-l-[8px] border-l-4 md:border-l-blue-600 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.05)] mb-8 md:mb-12 animate-in fade-in slide-in-from-bottom-8 duration-500",
    )
            ...
            rx.el.div(
                rx.el.div(
                    rx.el.div(
                        rx.el.div(
                            rx.icon(
                                "school", class_name="h-6 md:h-8 w-6 md:w-8 text-white"
                            ),
                            class_name="p-3 md:p-4 bg-white/20 rounded-xl md:rounded-2xl backdrop-blur-md border border-white/30 mr-4 md:mr-6",
                        ),
                        rx.el.div(
                            rx.el.h1(
                                "Select Higher Education Institution",
                                class_name="text-2xl md:text-4xl font-bold text-white tracking-tighter",
                            ),
                            class_name="flex-1",
                        ),
                        class_name="flex items-center",
                    ),
                    class_name="max-w-5xl mx-auto px-6 md:px-10 py-8 md:py-12",
                ),
                class_name="relative overflow-hidden rounded-2xl md:rounded-3xl bg-gradient-to-r from-blue-900 via-blue-800 to-indigo-900 shadow-2xl mb-8 md:mb-12",
            ),
            rx.cond(
                    ...
                    rx.cond(
                        ...
                        rx.el.div(
                            rx.el.div(
                                rx.el.div(
                                    rx.cond(
                                        ...
                                        rx.el.div(
                                            class_name="absolute left-4 md:left-5 top-4 md:top-5 h-5 md:h-6 w-5 md:w-6 border-2 border-blue-500 border-t-transparent rounded-full animate-spin z-10"
                                        ),
                                        rx.icon(
                                            "search",
                                            class_name="absolute left-4 md:left-5 top-4 md:top-5 text-slate-400 h-5 md:h-6 w-5 md:w-6 z-10",
                                        ),
                                    ),
                                    rx.el.input(
                                    ...
                                    rx.cond(
                                        ...
                                        rx.el.button(
                                            rx.icon(
                                                "x", class_name="h-4 md:h-5 w-4 md:w-5"
                                            ),
                                            on_click=HEIState.clear_search,
                                            class_name="absolute right-4 md:right-6 top-4 md:top-5 text-slate-400 hover:text-slate-600 transition-colors",
                                        ),
                                    ),
                            ...
                            rx.el.div(
                                ...
                                rx.el.div(class_name="h-px flex-1 bg-slate-100"),
                                class_name="flex items-center justify-center gap-4 mb-8 md:mb-12",
                            ),
                        ),
                    )
                ),
        ...
        rx.cond(
            ...
            rx.el.div(
                rx.el.div(
                    rx.icon(
                        "award",
                        class_name="h-5 md:h-6 w-5 md:w-6 text-indigo-600 mr-2 md:mr-3",
                    ),
                    rx.el.h2(
                        "Ranking Framework",
                        class_name="text-xl md:text-2xl font-black text-gray-900",
                    ),
                    class_name="flex items-center mb-6 md:mb-8 pb-4 border-b border-slate-100",
                ),
                rx.el.div(
                    ranking_framework_option(
                        value="QS",
                        label="QS World Rankings",
                        description="Evaluates based on academic reputation, employer reputation, and faculty impact.",
                        icon="sparkles",
                    ),
                    class_name="grid grid-cols-1 gap-4 md:gap-8 mb-8 md:mb-12 animate-in fade-in slide-in-from-bottom-10 duration-700",
                ),
                rx.el.button(
                    rx.cond(
                        HEIState.is_loading,
                        rx.el.div(
                            rx.el.div(
                                class_name="animate-spin rounded-full h-5 md:h-6 w-5 md:w-6 border-b-2 border-white mr-3"
                            ),
                            "Syncing Selection...",
                    ...
                    class_name=rx.cond(
                        ...
                        "w-full py-4 md:py-6 bg-gradient-to-r from-blue-700 to-indigo-800 hover:from-blue-800 hover:to-indigo-900 text-white rounded-2xl md:rounded-[2rem] font-black text-base md:text-xl shadow-[0_20px_40px_-15px_rgba(37,99,235,0.4)] transition-all transform active:scale-[0.98]",
                        "w-full py-4 md:py-6 bg-slate-200 text-slate-400 rounded-2xl md:rounded-[2rem] font-black text-base md:text-xl cursor-not-allowed",
                    ),
                ),
            ),
        ),
        class_name="max-w-5xl mx-auto w-full px-4 md:px-6",
    )