...
# Google AI API Configuration - with graceful fallback if package not installed
try:
...
"""
    ...
    logging.warning("google-generativeai package not installed. Using fallback recommendations.")
    GOOGLE_AI_AVAILABLE = False
except Exception as e:
    logging.warning(f"Error configuring Google AI: {e}. Using fallback recommendations.")
    GOOGLE_AI_AVAILABLE = False


class AnalyticsState(rx.State):
    # Category scores (out of 100)
    research_score: int = 0  # 50% weight
    employability_score: int = 0  # 20% weight
    global_engagement_score: int = 0  # 15% weight
    learning_experience_score: int = 0  # 10% weight
    sustainability_score: int = 0  # 5% weight
    overall_score: int = 0
    ...
    async def on_load(self):
        ...
        b_faculty_student_ratio = 12.0  # 1:12 is good
        b_sustainability = 85.0

        # Normalize scores (0-100 scale)
        s_academic_rep = min(100, academic_rep / b_academic_rep * 100) if b_academic_rep else 0
        ...
        s_emp_outcomes = min(100, emp_outcomes / b_emp_outcomes * 100) if b_emp_outcomes else 0
        s_int_research_net = min(100, int_research_net / b_int_research_net * 100) if b_int_research_net else 0
        s_int_faculty_ratio = min(100, int_faculty_ratio / b_int_faculty_ratio * 100) if b_int_faculty_ratio else 0
        s_int_student_ratio = min(100, int_student_ratio / b_int_student_ratio * 100) if b_int_student_ratio else 0
        # For faculty-student ratio, lower is better (1:12 is better than 1:20)
        # So we invert: if ratio is 1:12, score is 100; if 1:24, score is 50
        s_faculty_student = min(100, (b_faculty_student_ratio / faculty_student_ratio * 100)) if faculty_student_ratio > 0 else 0
        s_sustainability = min(100, sustainability / b_sustainability * 100) if b_sustainability else 0

        # Calculate category scores using weights
        # Research: Academic Reputation 30% + Citations 20% = 50% total
        self.research_score = int(s_academic_rep * 0.6 + s_citations * 0.4)  # 30/50 = 0.6, 20/50 = 0.4
        
        # Employability: Employer Reputation 15% + Employment Outcomes 5% = 20% total
        self.employability_score = int(s_emp_rep * 0.75 + s_emp_outcomes * 0.25)  # 15/20 = 0.75, 5/20 = 0.25
        
        # Global Engagement: Research Network 5% + Faculty Ratio 5% + Student Ratio 5% = 15% total
        self.global_engagement_score = int(
            (s_int_research_net + s_int_faculty_ratio + s_int_student_ratio) / 3
        )
        
        # Learning Experience: Faculty-Student Ratio 10% = 10% total
        self.learning_experience_score = int(s_faculty_student)
        
        # Sustainability: Sustainability Metrics 5% = 5% total
        self.sustainability_score = int(s_sustainability)

        # Calculate overall score using category weights
        # Research 50% + Employability 20% + Global Engagement 15% + Learning Experience 10% + Sustainability 5% = 100%
        self.overall_score = int(
            self.research_score * 0.50 +
            self.employability_score * 0.20 +
            self.global_engagement_score * 0.15 +
            self.learning_experience_score * 0.10 +
        ...
        # Set initial fallback recommendations immediately (synchronously)
        ...
        # Generate AI recommendations (non-blocking, will update if successful)
    ...
    async def generate_ai_recommendations(
        ...
        # If Google AI is not available, keep existing fallback recommendations
...
Return ONLY valid JSON, no additional text."""
            ...
            async with self:
                self.ai_recommendations = self._get_fallback_recommendations(
                    research_score, employability_score, global_engagement_score,
                    learning_experience_score, sustainability_score
                )
                self.is_generating_recommendations = False
    ...
    def _get_fallback_recommendations(
        ...
        # If no data entered (all scores are 0), provide general recommendations
        total_score = research_score + employability_score + global_engagement_score + learning_experience_score + sustainability_score
        if total_score == 0:
            recommendations.append({
